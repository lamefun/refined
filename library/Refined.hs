...
-- |
-- A predicate, which ensures that a value is inbetween or equal to either of the specified type-level numbers.
data FromTo (mn :: Nat) (mx :: Nat)

instance (Ord x, Num x, KnownNat mn, KnownNat mx) => Predicate (FromTo mn mx) x where
  validate p x =
